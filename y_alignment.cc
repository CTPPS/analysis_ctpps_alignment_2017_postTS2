...
TGraphErrors* BuildModeGraph(const TH2D *h2_y_vs_x)
{
	...
	for (int bix = 1; bix <= h2_y_vs_x->GetNbinsX(); ++bix)
	{
		...
		*/
		...
		if (fabs(y_mode_unc) > 1. || ff_gauss->GetChisquare() / ff_gauss->GetNDF() > 4.)
			continue;
...
int main()
{
	...
	// TODO: update sh_x - in 56 fill dependent...
	vector<RPData> rpData = {
		{ "L_2_F",  23, "sector 45", (cfg.xangle == 150) ? 0.150 : 0.140, -42.05 },
		{ "L_1_F",   3, "sector 45", (cfg.xangle == 150) ? 0.170 : 0.155, -3.45 },
		{ "R_1_F", 103, "sector 56", (cfg.xangle == 150) ? 0.225 : 0.210, -2.5 },
		{ "R_2_F", 123, "sector 56", (cfg.xangle == 150) ? 0.230 : 0.210, -42.1 }
	};
